...
- 巡洋舰(巡)×3：移动=正交直线1~2格，不可跳越，移动与攻击互斥。攻击（不消耗回合）：沿正交直线锁定一格（可为空格），路径至多越过1颗棋子（不分敌我）；锁定位置双方公开可见（棋盘红色四角框）。攻击必定在敌方下回合结束时结算：该格有舰船则击沉（潜艇免疫；敌护卫舰站上射击线则拦截）。每回合可令多艘巡洋舰各开火一次；开火的巡洋舰当回合不能移动。
...
回合：一方回合内可先令任意艘巡洋舰开火（不结束回合），然后移动一个棋子（结束回合）。
开局（红方，蓝方以海域分界镜像）：y0行：机(2,0)警(3,0)护(5,0)旗(6,0)护(7,0)警(9,0)机(10,0)；y2行：巡(0,2)(6,2)(12,2)；y4行：潜(3,4)(9,4)；y5行：驱(0,5)(2,5)(4,5)(6,5)(8,5)(10,5)(12,5)。
''';
