...
class AIDecision {
  ...
  final double elapsedSec; // API调用耗时（秒）
  AIDecision({required this.actions, this.undoSteps, required this.rawContent, this.elapsedSec = 0});
}
...
class AIClient {
  ...
  AIClient({required this.side})
      : _baseUrl = AppSettings.baseUrl,
        ...
        _model = AppSettings.model;

  /// 使用AI2配置（观AI战第二个AI）
  AIClient.forAI2({required this.side})
      : _baseUrl = AppSettings.ai2BaseUrl,
        _apiKey = AppSettings.ai2ApiKey,
        _model = AppSettings.ai2Model;
...
- **巡洋舰**(巡)×3：移动=正交直线1~2格（不可跳越），移动与攻击互斥。
  ...
  锁定位置双方公开可见（红色四角框），攻击必定在敌方下回合结束时结算：目标格有舰船则击沉（潜艇/战机/预警机免疫；敌护卫舰站射击线则拦截）。每回合可多艘各开火一次；开火者当回合不能移动。
...
**【轰炸策略】** 优先锁定炮架后方有敌方棋子的格子（可直接击沉敌舰）。轰炸空格仅在预判敌方走位封锁退路时选，勿反复轰炸无人空格。切勿将自己计划移动过去的格子作为轰炸目标。
