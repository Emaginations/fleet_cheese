...
- 巡洋舰(巡)×3：移动=正交直线1~2格，不可跳越，移动与攻击互斥。攻击（炮架规则，不消耗回合）：射击线上须有1颗棋子作炮架（不分敌我），有效目标为炮架自身及其后方直至下一颗棋子之前的所有位置（含空格）；锁定位置双方公开可见（红色四角框）。攻击必定在敌方下回合结束时结算：该格有舰船则击沉（潜艇免疫；敌护卫舰站上射击线则拦截）。每回合可令多艘巡洋舰各开火一次；开火的巡洋舰当回合不能移动。
